...
@pytest.mark.asyncio
async def test_get_all_bookmarks_authorized(async_client: AsyncClient, seed_data):
    response = await async_client.get(
        "/bookmarks/", headers={"Authorization": "Bearer testtoken"}
    )
...
@pytest.mark.asyncio
async def test_get_bookmark_authorized(async_client: AsyncClient, seed_data):
    response = await async_client.get(
        "/bookmarks/1", headers={"Authorization": "Bearer testtoken"}
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
async def test_create_bookmark(async_client: AsyncClient, db_session, seed_data):
    request_data = {
        "title": "Create Test",
    ...
    response = await async_client.post(
        "/bookmarks/", json=request_data, headers={"Authorization": "Bearer testtoken"}
    )
    ...
    s3_key = data["s3_key"]
    assert s3_key.startswith("user/1/")
    assert s3_key.endswith(".png")
    async_client.mock_send_task.assert_called_once_with(
        "bookmark.page_screenshot",
        args=["https://example.com/", s3_key],
    )
...
@pytest.mark.asyncio
async def test_update_bookmark(async_client: AsyncClient, db_session, seed_data):
    request_data = {
        "title": "Updated title",
        "url": "https://example.com/",
        "description": None,
    ...
    response = await async_client.put(
        "/bookmarks/1", json=request_data, headers={"Authorization": "Bearer testtoken"}
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
