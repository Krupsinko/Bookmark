...
@pytest.mark.asyncio
async def test_authenticate_user(seed_data, db_session):
    username = "test"
    password = "x"
...
def test_create_jwt(monkeypatch: MonkeyPatch):
    ...
    assert payload["exp"] == int(
        datetime(2025, 1, 1, 12, 30, tzinfo=timezone.utc).timestamp()
    )
...
async def test_login(
    seed_data, db_session, monkeypatch: MonkeyPatch, async_client: AsyncClient
):
    monkeypatch.setattr(users, "ALGORITHM", "HS256")
    monkeypatch.setattr(users, "SECRET_KEY", "testkey")

    request_data = {"username": "test", "password": "x"}

    response = await async_client.post("/user/token", data=request_data)
...
@pytest.mark.asyncio
async def test_create_user(db_session, async_client: AsyncClient):
    request_data = {
